...
function Library:change_theme(toTheme)
	...
	self:_refreshGlassStyles(toTheme)
end
...
function Library:_applyGlassStyle(guiWrapper, slotName)
	...
	table.insert(self.GlassObjects, {
		...
		stroke = stroke.AbsoluteObject,
		slotName = slotName
	})
	self:_refreshGlassStyles(self.CurrentTheme)
end
...
function Library:object(class, properties)
	...
	local frostSlot
	local frostSlotApplied = false
	local function applyFrostSlotStyling()
		if frostSlotApplied or not frostSlot then
			return
		end
		local st = Library.CurrentTheme.SlotTransparency
		local trans = (st and st[frostSlot] ~= nil) and st[frostSlot] or 0
		localObject.BackgroundTransparency = trans
		table.insert(Library.ThemeTransparencyObjects, {methods, frostSlot})
		Library:_applyGlassStyle(methods, frostSlot)
		frostSlotApplied = true
	end
	...
	local customHandlers = {
		...
		FrostSlot = function(value)
			frostSlot = value
			applyFrostSlotStyling()
		end,
	}
